...
void compute_order(int no)
{
    ...
    const int eta_max=10;
    if(no<100)
    {
        a=200;b=5;eps=0.1;repeat=80;
    }

    else if(no<1000 && no>=100)
    {
        a=200;b=5;eps=0.1;repeat=10;
    }

    else if(no>=1000)
    {
        a=20;b=5;eps=0.1;repeat=1;
    }
    ...
    for(int r=0;r<repeat;r++)
    {
        ...
        for(int i=0;i<no_points;i++)
        {
            ...
            for(t=0;t<MAX_TIME;t++)
            {
                if(t>a)
                {
                    if(fabs(array_binder(ordall,t-2*b,t-b)-
                            array_binder(ordall,t-b,t))<eps)
                        break;
                }
                theta_cos = 0.;
                theta_sin = 0.;
            ...
            order_arr[i] += array_binder(ordall,t-2*b,t);
        }
    }
    for(int i=0;i<no_points;i++)
    {
